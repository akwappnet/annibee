...
class UpcomingAnniversaryDetailAdapter(
    val callback: (item: AnniversaryEvent) -> Unit
) :
    RecyclerView.Adapter<UpcomingAnniversaryDetailAdapter.ViewHolder>() {
    ...
    class ViewHolder(
        ...
        val adapter: UpcomingAnniversaryDetailAdapter
        ...
        fun bind(data: AnniversaryEvent) {
            ...
            when (data.type) {
                "1" -> {
                    binding.txtTitle.setTextColor(ContextCompat.getColor(itemView.context, R.color.sky_blue))
                }
                "2" -> {
                    binding.txtTitle.setTextColor(ContextCompat.getColor(itemView.context, R.color.light_brown))
                }
                else -> {
                    binding.txtTitle.setTextColor(ContextCompat.getColor(itemView.context, R.color.green))
                }
            }
        }
    }
